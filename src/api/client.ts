...
async function request<Route extends ApiRoute>(route: Route, options: RequestOptions = {}): Promise<ApiResponseMap[Route]> {
  ...
  const response = await fetch(`${API_BASE}${route}`, { method: options.method ?? 'GET', headers, body: options.body });
...
export const apiClient = {
  ...
  identity: () => request('/identity'), umbrella: () => request('/umbrella'), sim: () => request('/sim'), kernel: () => request('/kernel'),
  windows: () => request('/windows'), bridge: () => request('/bridge'), processTree: () => request('/process-tree'),
  windowsList: (): Promise<WindowInfo[]> => request('/windows' as ApiRoute) as Promise<WindowInfo[]>, simAgents: () => request('/sim/agents'),
  umbrellaRules: () => request('/umbrella/rules'), toggleUmbrellaRule: (id: string) => request('/umbrella/rules/:id/toggle'.replace(':id', encodeURIComponent(id)) as '/umbrella/rules/:id/toggle', { method: 'POST' }),
  umbrellaCompile: (rule: string) => request('/umbrella/compile', { method: 'POST', body: JSON.stringify({ rule }) }),
  kernelRestart: () => request('/kernel/restart', { method: 'POST' }), kernelKillProcess: (pid: string) => request('/kernel/kill/:pid'.replace(':pid', encodeURIComponent(pid)) as '/kernel/kill/:pid', { method: 'POST' }),
